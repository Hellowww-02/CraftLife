...
def get_conn():
    c = sqlite3.connect(DB_PATH, timeout=60.0, cached_statements=100)
...
def init_db():
    ...
    if table_check:
        ...
        if user_count > 0:
            print(f"[DB] Database sudah terisi ({user_count} user), skip data migration.")
            # Tetap lanjutkan ke commit tanpa data migration
            c.execute("PRAGMA foreign_keys = ON")
            conn.commit()
            conn.close()
...
def add_habit(user_id, name, icon="⚔️", difficulty="medium",
    ...
    conn = get_conn()
    # Hitung sort_order terakhir untuk user ini tanpa folder (folder_id NULL)
...
SHOP_ITEMS = {
    ...
    # ── Special ──
    "ice_block":       {"name": "Ice Block",       "icon": "🧊", "cost": 100,
...
def delete_debt(debt_id, user_id):
    # Jika hutang sudah lunas, jangan hapus transaksi expense yang sudah dibuat
    # Cukup hapus hutangnya saja
    conn = get_conn()
...
def log_recipe(user_id, recipe_id, serving_multiplier, meal_type, log_date, notes=""):
    ...
    # Simpan sebagai satu entri log dengan nama resep (bisa pilih apakah ingin simpan per item? lebih sederhana satu entri)
    # Tapi agar akurat, kita simpan per item (biar analisis nutrisi detail). Namun untuk kemudahan, kita simpan satu entri dengan total nutrisi.
    # Untuk track bahan detail, lebih baik simpan per item. Tapi karena food_logs sudah ada foreign key ke food_items, kita simpan per item.
    # Implementasi: buat multiple log entries, satu per item.
    conn = get_conn()
    for item in details["items"]:
        qty = item["quantity"] * serving_multiplier
        conn.execute("""
...
def update_note(note_id, user_id, title=None, content=None, folder_id=None):
    updates = []
    values = []
...
def force_checkpoint():
    """Paksa semua data dari WAL ke database utama."""
    try:
        conn = get_conn()
        conn.execute("PRAGMA wal_checkpoint(TRUNCATE)")
        conn.close()
        print("[DB] Checkpoint force berhasil.")
    except Exception as e:
        print(f"[DB] Checkpoint force gagal: {e}")

def backup_database():
    """Buat backup database ke folder backups/ dengan timestamp."""
    try:
        force_checkpoint()  # pastikan semua data tertulis
        src = DB_PATH
        if not os.path.exists(src):
            return None
        backup_dir = os.path.join(os.path.dirname(src), "backups")
