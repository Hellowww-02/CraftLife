...
def get_conn():
    c = sqlite3.connect(DB_PATH, timeout=60.0, cached_statements=100)  # timeout 60 detik
    c.row_factory = sqlite3.Row
    c.execute("PRAGMA foreign_keys = ON")
    c.execute("PRAGMA journal_mode = WAL")
    c.execute("PRAGMA busy_timeout = 60000")  # 60 detik
...
def require_unlocked(func):
    """Decorator untuk memastikan akun tidak dilock sebelum menjalankan fungsi."""
    @functools.wraps(func)
    def wrapper(user_id, *args, **kwargs):
        # Cek apakah parameter pertama adalah user_id (kebanyakan fungsi)
        if is_account_locked(user_id):
            return {"ok": False, "msg": tr_db(user_id=user_id, key="db_account_locked_msg")}
...
def calculate_rank(user_id):
    ...
    u = get_user(user_id)
    if not u:
        return {"rank": 0, "rank_name": "Pemula", "rank_icon": "🥚", "score": 0, "max_score": 100}
