...
def init_db():
    ...
    _safe_alter(c, "relationship_profiles", "my_name", "TEXT DEFAULT ''")
    _safe_alter(c, "relationship_profiles", "my_gender", "TEXT DEFAULT 'male'")
    _safe_alter(c, "relationship_profiles", "my_age", "INTEGER DEFAULT 25")
    _safe_alter(c, "relationship_profiles", "my_birthdate", "TEXT")
    _safe_alter(c, "relationship_profiles", "partner_birthdate", "TEXT")
    # ── Love Space Gallery: album pribadi & bersama pasangan ──
    c.execute("""CREATE TABLE IF NOT EXISTS love_albums(
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER NOT NULL,
        name TEXT NOT NULL,
    ...
    c.execute("CREATE INDEX IF NOT EXISTS idx_love_album_items_photo ON love_album_items(photo_id)")
    # ── Performa: index pencarian foto per pemilik & album ──
    c.execute("CREATE INDEX IF NOT EXISTS idx_love_photos_owner ON love_space_photos(owner_user_id)")
    c.execute("CREATE INDEX IF NOT EXISTS idx_love_photos_space ON love_space_photos(love_space_id)")
...
def get_love_space_photo_meta(requester_user_id, limit=1000):
    """Metadata foto TANPA BLOB (image_data) — jauh lebih cepat untuk daftar galeri.
    Struktur hasil sama dengan get_love_space_photos minus 'image_data' & 'size_bytes'."""
    relationship = get_active_couple_relationship(requester_user_id)
    love_space_id = relationship.get("love_space_id") if relationship else None
    cols = ("p.id,p.love_space_id,p.owner_user_id,p.visibility,p.width,p.height,"
            "p.mime_type,p.caption,p.photo_date,p.created_at,p.cloud_id,p.sync_status,"
            "u.display_name AS uploader_name")
    conn=get_conn()
    if love_space_id and _love_space_membership(conn, requester_user_id, love_space_id):
        rows=conn.execute(f"""SELECT {cols} FROM love_space_photos p
            JOIN users u ON u.id=p.owner_user_id
            WHERE (p.owner_user_id=?)
               OR (p.visibility='shared' AND p.love_space_id=?
                   AND EXISTS(SELECT 1 FROM love_space_members m WHERE m.love_space_id=p.love_space_id AND m.user_id=?))
...
def get_love_space_photo(requester_user_id, photo_id):
    # Performa: ambil langsung per id (jangan muat semua foto + blob).
    conn = get_conn()
    row = conn.execute(
        "SELECT p.*, u.display_name AS uploader_name FROM love_space_photos p "
        "JOIN users u ON u.id=p.owner_user_id WHERE p.id=?", (photo_id,)).fetchone()
    conn.close()
    if not row:
        return None
    item = dict(row); item["image_data"] = bytes(item["image_data"])
    scope = _love_scope_user_ids(requester_user_id)
    if int(item["owner_user_id"]) == int(requester_user_id):
        return item
    if item.get("visibility") == "shared" and item.get("love_space_id"):
        if int(requester_user_id) in scope:
            return item
    return None
...
def update_health_goals(user_id, daily_steps, daily_sleep_hours, height_cm=None, weight_kg=None):
    ...
    # FIX: sebelumnya INSERT OR REPLACE menghapus baris lalu insert baru -> kolom
    # age/gender/activity_factor ikut ter-reset ke default (umur balik ke 25).
    # Sekarang UPDATE in-place agar data BMI/profil tidak hilang.
    conn.execute("""
        INSERT INTO user_health_goals(user_id, daily_steps, daily_sleep_hours, height_cm, weight_kg, updated_at)
        VALUES(?,?,?,?,?,datetime('now'))
        ON CONFLICT(user_id) DO UPDATE SET
            daily_steps=excluded.daily_steps,
            daily_sleep_hours=excluded.daily_sleep_hours,
...
def get_relationship_profile(user_id):
    ...
    # Sisi "kamu" mengikuti perspektif user yang melihat, supaya cowo & cewe
    # masing-masing melihat datanya di sisi yang benar pada profil bersama.
    relationship = get_active_couple_relationship(user_id)
    if relationship and (relationship.get("user_a_id") == user_id or relationship.get("user_b_id") == user_id):
        other = relationship["user_b_id"] if relationship["user_a_id"] == user_id else relationship["user_a_id"]
        me = get_user(user_id) or {}
        other_u = get_user(other) or {}
        # Data "my_*" pada baris tersimpan relatif ke primary user; putar bila perlu.
        stored_my = profile.get("my_name") or ""
        stored_my_gender = profile.get("my_gender") or ""
        stored_my_age = profile.get("my_age")
        stored_my_birthdate = profile.get("my_birthdate")
        stored_partner = profile.get("partner_name") or ""
        stored_partner_birthdate = profile.get("partner_birthdate")
        if owner_id != user_id:
            # Pemirsa adalah secondary user → sisi tersimpan tertukar untuknya.
            profile["my_name"] = stored_partner
            profile["my_gender"] = profile.get("partner_gender")
            profile["my_age"] = profile.get("partner_age")
            profile["my_birthdate"] = stored_partner_birthdate
            profile["partner_name"] = stored_my
            profile["partner_gender"] = stored_my_gender
            profile["partner_age"] = stored_my_age
            profile["partner_birthdate"] = stored_my_birthdate
        # Auto-lengkapi dari akun asli bila sisi kosong.
        if not profile.get("my_name"):
            profile["my_name"] = me.get("display_name") or me.get("username") or ""
        if not profile.get("partner_name"):
            profile["partner_name"] = other_u.get("display_name") or other_u.get("username") or ""
    elif not profile.get("partner_name"):
        me = get_user(user_id) or {}
        profile["my_name"] = me.get("display_name") or me.get("username") or ""
    return profile
...
def save_relationship_profile(user_id, partner_name, partner_gender, partner_age,
                              ...
                              my_name="", my_gender="male", my_age=25, my_birthdate=None,
                              partner_birthdate=None):
    if is_account_locked(user_id):
        return {"ok": False, "msg": tr_db(user_id=user_id, key="db_account_locked_msg")}
    owner_id = _love_primary_user_id(user_id)
    try:
        my_age = int(my_age or 25)
    except (TypeError, ValueError):
        my_age = 25
    try:
        partner_age = int(partner_age or 25)
    except (TypeError, ValueError):
        partner_age = 25
    # Baris tersimpan relatif ke primary user. Bila penyimpan adalah secondary,
    # tukar sisi agar data tetap konsisten untuk kedua akun.
    if owner_id != int(user_id):
        my_name, partner_name = partner_name, my_name
        my_gender, partner_gender = partner_gender, my_gender
        my_age, partner_age = partner_age, my_age
        my_birthdate, partner_birthdate = partner_birthdate, my_birthdate
    conn = get_conn()
    conn.execute("""
        INSERT INTO relationship_profiles(user_id, partner_name, partner_gender, partner_age,
                                          relationship_type, start_date, my_name, my_gender, my_age,
                                          my_birthdate, partner_birthdate, updated_at)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,datetime('now'))
        ON CONFLICT(user_id) DO UPDATE SET
            ...
            start_date=excluded.start_date, my_name=excluded.my_name, my_gender=excluded.my_gender,
            my_age=excluded.my_age, my_birthdate=excluded.my_birthdate,
            partner_birthdate=excluded.partner_birthdate, updated_at=datetime('now')
    """, (owner_id, (partner_name or "").strip(), partner_gender, partner_age,
          relationship_type, start_date, (my_name or "").strip(), my_gender, my_age,
          my_birthdate, partner_birthdate))
    conn.commit(); conn.close()
    return {"ok": True}
...
def mirror_cloud_love_profile(local_user_id,row):
    ...
    conn.execute("""INSERT INTO relationship_profiles(
        user_id,partner_name,partner_gender,partner_age,relationship_type,start_date,
        my_name,my_gender,my_age,my_birthdate,partner_birthdate,
        updated_at,cloud_id,cloud_updated_at
      ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)
      ...
      (owner,row.get("partner_name") or "",row.get("partner_gender") or "female",int(row.get("partner_age") or 25),
       row.get("relationship_type") or "dating",row.get("start_date"),
       row.get("my_name") or "",row.get("my_gender") or "male",int(row.get("my_age") or 25),
       row.get("my_birthdate"),row.get("partner_birthdate"),
       row.get("updated_at") or local_now().isoformat(), space, row.get("updated_at")))
...
def add_supply_item(user_id, name, category="", unit="pcs", stock=0, min_stock=0, price=0, location="", notes="", economy_category=""):
    if not (name or "").strip():
        return {"ok": False, "code": "name"}
    conn = get_conn()
    cur = conn.execute("""INSERT INTO supplies_items(user_id,name,category,unit,stock,min_stock,price,location,notes,economy_category)
                          VALUES(?,?,?,?,?,?,?,?,?,?)""",
                       (user_id, name.strip(), category or "", unit or "pcs", stock or 0, min_stock or 0, price or 0, location or "", notes or "", economy_category or ""))
    iid = cur.lastrowid
    if stock:
...
def update_supply_item(user_id, item_id, **kw):
    allowed = {"name", "category", "unit", "stock", "min_stock", "price", "location", "notes", "economy_category"}
...
def get_learning_font_sizes(user_id):
    u = get_user(user_id) or {}
    try:
        chat = int(u.get("learning_chat_font") or 13)
    except (TypeError, ValueError):
        chat = 13
    try:
        studio = int(u.get("learning_studio_font") or 14)
    except (TypeError, ValueError):
        studio = 14
    return {"chat": max(10, min(30, chat)), "studio": max(10, min(30, studio))}


def set_learning_font_sizes(user_id, chat=None, studio=None):
    kw = {}
    if chat is not None:
        try:
            kw["learning_chat_font"] = max(10, min(30, int(chat)))
        except (TypeError, ValueError):
            pass
    if studio is not None:
        try:
            kw["learning_studio_font"] = max(10, min(30, int(studio)))
        except (TypeError, ValueError):
            pass
    if kw:
        update_user(user_id, **kw)
...
# ══════════════════════════════════════════════════════════════════════════════
#  LOVE SPACE GALLERY — ALBUM (pribadi & bersama pasangan, seperti galeri HP)
# ══════════════════════════════════════════════════════════════════════════════

def _love_album_accessible(user_id, album_row):
    if not album_row:
        return False
    if int(album_row["user_id"]) == int(user_id):
        return True
    if album_row.get("scope") == "shared":
        return int(user_id) in _love_scope_user_ids(user_id)
    return False


def create_love_album(user_id, name, scope="personal"):
    name = (name or "").strip()
    if not name:
        return {"ok": False, "code": "name"}
    scope = scope if scope in ("personal", "shared") else "personal"
    conn = get_conn()
    cur = conn.execute("INSERT INTO love_albums(user_id,name,scope) VALUES(?,?,?)", (user_id, name, scope))
    conn.commit(); aid = cur.lastrowid; conn.close()
    return {"ok": True, "id": aid}


def get_love_albums(user_id):
    scope_ids = _love_scope_user_ids(user_id)
    conn = get_conn()
    q = "SELECT * FROM love_albums WHERE (user_id=? AND scope='personal')"
    args = [int(user_id)]
    if len(scope_ids) > 1:
        q += " OR (scope='shared' AND user_id IN (%s))" % ",".join("?" * len(scope_ids))
        args += [int(i) for i in scope_ids]
    q += " ORDER BY scope DESC, name COLLATE NOCASE"
    rows = conn.execute(q, args).fetchall()
    conn.close()
    return [dict(r) for r in rows]


def get_love_album(album_id):
    conn = get_conn()
    row = conn.execute("SELECT * FROM love_albums WHERE id=?", (album_id,)).fetchone()
    conn.close()
    return dict(row) if row else None


def rename_love_album(user_id, album_id, name):
    name = (name or "").strip()
    if not name:
        return {"ok": False, "code": "name"}
    ...
    conn = get_conn()
    conn.execute("UPDATE love_albums SET name=?, updated_at=datetime('now') WHERE id=?", (name, album_id))
    conn.commit(); conn.close()
    return {"ok": True}


def delete_love_album(user_id, album_id):
    album = get_love_album(album_id)
    if not _love_album_accessible(user_id, album):
        return {"ok": False, "code": "forbidden"}
    conn = get_conn()
    conn.execute("DELETE FROM love_album_items WHERE album_id=?", (album_id,))
    cur = conn.execute("DELETE FROM love_albums WHERE id=?", (album_id,))
    conn.commit(); ok = cur.rowcount; conn.close()
    return {"ok": bool(ok)}


def _love_photo_visible_to(user_id, photo_id):
    return get_love_space_photo(user_id, photo_id)


def add_photo_to_love_album(user_id, album_id, photo_id):
    if not _love_album_accessible(user_id, get_love_album(album_id)):
        return {"ok": False, "code": "album"}
    if not _love_photo_visible_to(user_id, photo_id):
        return {"ok": False, "code": "photo"}
    conn = get_conn()
    conn.execute("INSERT OR IGNORE INTO love_album_items(album_id,photo_id) VALUES(?,?)", (album_id, photo_id))
    conn.commit(); conn.close()
    return {"ok": True}


def remove_photo_from_love_album(user_id, album_id, photo_id):
    if not _love_album_accessible(user_id, get_love_album(album_id)):
        return {"ok": False, "code": "album"}
    conn = get_conn()
    conn.execute("DELETE FROM love_album_items WHERE album_id=? AND photo_id=?", (album_id, photo_id))
    conn.commit(); conn.close()
    return {"ok": True}


def move_photo_to_love_album(user_id, source_album_id, target_album_id, photo_id):
    """source_album_id 0/None = dari 'Semua foto' (hanya salin)."""
    if source_album_id == target_album_id:
        return {"ok": True}
    if not _love_album_accessible(user_id, get_love_album(target_album_id)):
        return {"ok": False, "code": "album"}
    if source_album_id and not _love_album_accessible(user_id, get_love_album(source_album_id)):
        return {"ok": False, "code": "album"}
    if not _love_photo_visible_to(user_id, photo_id):
        return {"ok": False, "code": "photo"}
    conn = get_conn()
    if source_album_id:
        conn.execute("DELETE FROM love_album_items WHERE album_id=? AND photo_id=?", (source_album_id, photo_id))
...
def get_love_album_photos(user_id, album_id):
    conn = get_conn()
    ids = [r[0] for r in conn.execute("SELECT photo_id FROM love_album_items WHERE album_id=?", (album_id,)).fetchall()]
    conn.close()
    photos = get_love_space_photos(user_id, 1000)
    by_id = {p["id"]: p for p in photos}
    return [by_id[i] for i in ids if i in by_id]


def get_love_album_photo_ids(user_id, album_id):
    """ID foto milik album (tanpa memuat blob) — untuk listing cepat."""
    conn = get_conn()
    rows = conn.execute("SELECT photo_id FROM love_album_items WHERE album_id=?", (album_id,)).fetchall()
    conn.close()
    return [r[0] for r in rows]


def get_photo_album_ids(user_id, photo_id):
    conn = get_conn()
    rows = conn.execute("SELECT album_id FROM love_album_items WHERE photo_id=?", (photo_id,)).fetchall()
    conn.close()
    out = []
    for r in rows:
        album = get_love_album(r[0])
        if _love_album_accessible(user_id, album):
            out.append(album["id"])
    return out


# ══════════════════════════════════════════════════════════════════════════════
#  SUPPLIES — INTEGRASI FULL-LINK DENGAN ECONOMY
# ══════════════════════════════════════════════════════════════════════════════

def get_low_stock_supplies(user_id, limit=50):
    conn = get_conn()
    rows = conn.execute("""SELECT * FROM supplies_items
        WHERE user_id=? AND min_stock>0 AND stock<=min_stock
        ORDER BY stock ASC LIMIT ?""", (user_id, limit)).fetchall()
    conn.close()
    return [dict(r) for r in rows]


def supplies_stats_by_category(user_id):
    conn = get_conn()
    rows = conn.execute("""SELECT category, COUNT(*) AS items, COALESCE(SUM(stock*price),0) AS value,
        SUM(CASE WHEN stock<=min_stock AND min_stock>0 THEN 1 ELSE 0 END) AS low
        FROM supplies_items WHERE user_id=? GROUP BY category ORDER BY category COLLATE NOCASE""", (user_id,)).fetchall()
    conn.close()
    return [dict(r) for r in rows]


def add_supplies_expense(user_id, item_name, amount, category="Supplies", date_str=None, notes=""):
    try:
        amount = float(amount)
    except (TypeError, ValueError):
    ...
    if amount <= 0:
        return {"ok": False, "code": "amount"}
    add_economy_item(user_id, item_name or "Supplies", "🛒", "expense", amount,
                     category or "Supplies", date_str or date.today().isoformat(), notes or "")
    return {"ok": True}


def record_supply_tx_with_economy(user_id, item_id, kind, qty, note="",
                                  log_economy=False, economy_amount=0, economy_category=""):
    result = record_supply_tx(user_id, item_id, kind, qty, note)
    if result.get("ok") and log_economy:
        try:
            amount = float(economy_amount or 0)
        except (TypeError, ValueError):
            amount = 0
        if amount > 0:
            conn = get_conn()
            item = conn.execute("SELECT * FROM supplies_items WHERE id=?", (item_id,)).fetchone()
            conn.close()
            item = dict(item) if item else {}
            name = item.get("name") or "Supplies"
            category = economy_category or item.get("economy_category") or "Supplies"
            add_economy_item(user_id, name, "🛒", "expense", amount,
                             category, date.today().isoformat(), note or "")
    return result


# ══════════════════════════════════════════════════════════════════════════════
#  TEMPLATE KARTU TUGAS — Dailies / Quests / SportTrack (gaya template Habits)
#  Struktur daily/todo: (id, en, icon, difficulty) · sport: (id, en, sport_type, icon, difficulty)
# ══════════════════════════════════════════════════════════════════════════════

DAILY_TEMPLATES = [
    {
        "key": "morning_routine_d", "icon": "🌅",
        "name": ("Rutinitas Pagi", "Morning Routine"),
        "desc": ("Dailies ringan untuk memulai hari dengan benar",
                 "Light dailies to start the day right"),
        "items": [
            ("Minum air setelah bangun", "Drink water after waking up", "💧", "easy"),
            ("Rapikan tempat tidur", "Make the bed", "🛏️", "easy"),
            ("Sarapan sehat", "Healthy breakfast", "🍳", "easy"),
            ("Olahraga ringan 15 menit", "15 minutes light exercise", "🏃", "medium"),
            ("Review agenda hari ini", "Review today's agenda", "📋", "easy"),
        ],
    },
    {
        "key": "work_daily", "icon": "💼",
        "name": ("Produktivitas Kerja", "Work Productivity"),
        "desc": ("Dailies untuk hari kerja yang fokus",
                 "Dailies for a focused workday"),
        "items": [
            ("Cek email penting", "Check important emails", "📧", "easy"),
            ("Selesaikan 1 tugas prioritas", "Finish 1 priority task", "🎯", "medium"),
            ("Rapikan meja kerja", "Tidy your desk", "🗂️", "easy"),
            ("Update progres ke tim", "Update progress to the team", "👥", "easy"),
            ("Tutup hari: review & rencana besok", "Wrap up: review & plan tomorrow", "🌙", "medium"),
        ],
    },
    {
        "key": "self_care_daily", "icon": "🌿",
        "name": ("Perawatan Diri", "Self Care"),
        "desc": ("Dailies untuk menjaga kesehatan fisik & mental",
                 "Dailies to keep body and mind healthy"),
        "items": [
            ("Meditasi 5 menit", "5 minutes of meditation", "🧘", "easy"),
            ("Jalan kaki 20 menit", "20-minute walk", "🚶", "easy"),
            ("Tidur sebelum jam 11", "Sleep before 11 PM", "💤", "hard"),
            ("Sikat gigi sebelum tidur", "Brush teeth before bed", "🦷", "easy"),
            ("Baca 10 halaman", "Read 10 pages", "📖", "medium"),
        ],
    },
    {
        "key": "study_daily", "icon": "🎓",
        "name": ("Belajar Harian", "Daily Study"),
        "desc": ("Dailies untuk kebiasaan belajar konsisten",
                 "Dailies for consistent study habits"),
        "items": [
            ("Belajar fokus 30 menit", "30 minutes focused study", "📖", "medium"),
            ("Catat 3 poin penting", "Note 3 key points", "✏️", "easy"),
            ("Kerjakan latihan soal", "Do practice exercises", "🧩", "medium"),
            ("Review catatan kemarin", "Review yesterday's notes", "🔁", "easy"),
        ],
    },
    {
        "key": "household_daily", "icon": "🏠",
        "name": ("Urusan Rumah", "Household"),
        "desc": ("Dailies agar rumah tetap rapi dan teratur",
                 "Dailies to keep the home tidy"),
        "items": [
            ("Cuci piring", "Wash the dishes", "🍽️", "easy"),
            ("Bersihkan kamar 10 menit", "10 minutes tidying your room", "🧹", "easy"),
            ("Siapkan makanan", "Prepare a meal", "🍲", "medium"),
            ("Buang sampah", "Take out the trash", "🗑️", "easy"),
        ],
    },
]
...
TODO_TEMPLATES = [
    {
        "key": "project_launch_t", "icon": "🚀",
        "name": ("Persiapan Proyek", "Project Prep"),
        "desc": ("Quest bertahap untuk meluncurkan proyek",
                 "Staged quests to launch a project"),
        "items": [
            ("Riset kebutuhan proyek", "Research project requirements", "🔍", "medium"),
            ("Buat rencana kerja", "Create a work plan", "📋", "medium"),
            ("Siapkan proposal", "Prepare the proposal", "📄", "hard"),
            ("Review dengan tim", "Review with the team", "👥", "medium"),
            ("Kirim hasil akhir", "Submit the final result", "📮", "hard"),
        ],
    },
    {
        "key": "event_planning_t", "icon": "🎉",
        "name": ("Rencana Acara", "Event Planning"),
        "desc": ("Quest untuk menyiapkan sebuah acara",
                 "Quests to organize an event"),
        "items": [
            ("Tentukan tema & tanggal", "Set theme & date", "📅", "easy"),
            ("Buat daftar tamu", "Make the guest list", "📝", "easy"),
            ("Pesan venue/catering", "Book venue/catering", "📞", "medium"),
            ("Sebar undangan", "Send invitations", "✉️", "medium"),
            ("Konfirmasi kehadiran", "Confirm attendance", "✅", "medium"),
        ],
    },
    {
        "key": "learning_goals_t", "icon": "🎓",
        "name": ("Target Belajar", "Learning Goals"),
        "desc": ("Quest untuk menuntaskan materi baru",
                 "Quests to master new material"),
        "items": [
            ("Pilih materi baru", "Pick new material", "📚", "easy"),
            ("Selesaikan modul 1", "Finish module 1", "📖", "hard"),
            ("Buat rangkuman", "Write a summary", "✏️", "medium"),
            ("Kerjakan ujian latihan", "Take a practice exam", "🧩", "hard"),
            ("Review & evaluasi", "Review & evaluate", "🔁", "medium"),
        ],
    },
    {
        "key": "house_move_t", "icon": "📦",
        "name": ("Pindahan Rumah", "Moving House"),
        "desc": ("Quest untuk pindahan yang tertata",
                 "Quests for an organized move"),
        "items": [
            ("Sortir barang", "Sort belongings", "🗃️", "medium"),
            ("Packing kardus", "Pack boxes", "📦", "medium"),
            ("Sewa transportasi", "Book transport", "🚚", "hard"),
            ("Update alamat", "Update your address", "🏠", "medium"),
            ("Unpack & tata rumah", "Unpack & arrange the home", "🛋️", "hard"),
        ],
    },
]
...
SPORT_TEMPLATES = [
    {
        "key": "running_starter_s", "icon": "🏃",
        "name": ("Mulai Lari", "Running Starter"),
        "desc": ("Aktivitas lari dari pemula ke rutin",
                 "Running activities from beginner to routine"),
        "items": [
            ("Lari pagi 30 menit", "30-minute morning run", "running", "🏃", "medium"),
            ("Interval lari 5x200m", "5x200m sprint intervals", "running", "⏱️", "hard"),
            ("Jalan cepat 20 menit", "20-minute brisk walk", "running", "🚶", "easy"),
            ("Lari sore 15 menit", "15-minute evening run", "running", "🌇", "easy"),
            ("Long run 5 km", "5 km long run", "running", "🛣️", "hard"),
        ],
    },
    {
        "key": "gym_strength_s", "icon": "🏋️",
        "name": ("Kekuatan Gym", "Gym Strength"),
        "desc": ("Jadwal latihan beban terstruktur",
                 "A structured weight-training schedule"),
        "items": [
            ("Push day", "Push day", "gym", "🏋️", "hard"),
            ("Pull day", "Pull day", "gym", "🏋️", "hard"),
            ("Leg day", "Leg day", "gym", "🦵", "hard"),
            ("Full body ringan", "Light full body", "gym", "💪", "medium"),
        ],
    },
    {
        "key": "cycling_week_s", "icon": "🚴",
        "name": ("Bersepeda", "Cycling"),
        "desc": ("Aktivitas bersepeda santai hingga menanjak",
                 "Cycling from casual to climbing"),
        "items": [
            ("Sepeda santai 10 km", "10 km easy ride", "cycling", "🚴", "easy"),
            ("Sepeda 20 km", "20 km ride", "cycling", "🚵", "medium"),
            ("Hill climb 5 km", "5 km hill climb", "cycling", "⛰️", "hard"),
        ],
    },
    {
        "key": "yoga_flow_s", "icon": "🧘",
        "name": ("Yoga & Kelenturan", "Yoga & Flexibility"),
        "desc": ("Rutin yoga untuk kelenturan dan ketenangan",
                 "Yoga routines for flexibility and calm"),
        "items": [
            ("Yoga pagi 15 menit", "15-minute morning yoga", "yoga", "🧘", "easy"),
            ("Peregangan sebelum tidur", "Bedtime stretching", "yoga", "🌙", "easy"),
            ("Vinyasa 30 menit", "30-minute vinyasa", "yoga", "🧘", "medium"),
        ],
    },
    {
        "key": "swim_casual_s", "icon": "🏊",
        "name": ("Renang", "Swimming"),
        "desc": ("Aktivitas renang teknik & daya tahan",
                 "Swimming for technique & endurance"),
        "items": [
            ("Renang 20 putaran", "Swim 20 laps", "swimming", "🏊", "medium"),
            ("Latihan napas & teknik", "Breathing & technique drill", "swimming", "💨", "easy"),
            ("Renang interval 10x50m", "10x50m intervals", "swimming", "⏱️", "hard"),
        ],
    },
]
...
def get_templates_by_mode(mode, lang="id"):
    templates = TEMPLATE_BY_MODE.get(mode) or HABIT_TEMPLATES
    out = []
    for t in templates:
        out.append({
            "key": t["key"], "icon": t["icon"],
...
def apply_template_by_mode(user_id, mode, template_key):
    templates = TEMPLATE_BY_MODE.get(mode) or HABIT_TEMPLATES
    ...
    for item in template["items"]:
        if mode == "daily":
            add_daily(user_id, item[idx], item[2], item[3])
        elif mode == "todo":
            add_todo(user_id, item[idx], item[3], item[2])
        elif mode == "sport":
            add_sport_activity(user_id, item[idx], item[2], item[3], item[4])
        else:
            add_habit(user_id, item[idx], item[2], item[3])
        n += 1
    return n
