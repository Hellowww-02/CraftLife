-- CraftLife v1.6.3 · fase A10 — Love Space: kenangan & bucket list yang matang
--
...
--
-- Isi:
--   1. Kolom baru public.love_space_memories (emoji, tags, is_favorite, updated_at)
--      dan public.love_space_bucket_items (notes, priority, updated_at).
--      Catatan: `photo_id` kenangan & `promoted_memory_id` bucket list sengaja
--      TIDAK dikirim ke cloud — keduanya menunjuk data lokal (galeri lokal &
--      id kenangan hasil promosi), jadi cukup tersimpan di perangkat.
--   2. Definisi ulang public.upsert_love_space_record(...) dengan cabang 'memory' &
--      'bucket_item' yang menulis kolom baru (sisa cabang lain disalin apa adanya).
...
-- A10: kenangan (emoji pilihan, tag, favorit) & bucket list (catatan, prioritas).
alter table public.love_space_memories
  add column if not exists emoji text not null default '',
  add column if not exists tags text not null default '',
  add column if not exists is_favorite boolean not null default false,
  add column if not exists updated_at timestamptz not null default now();

alter table public.love_space_bucket_items
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'love_space_bucket_items_priority_check'
  ) then
    alter table public.love_space_bucket_items
      add constraint love_space_bucket_items_priority_check check (priority between 0 and 3);
  end if;
end $$;

create index if not exists idx_love_space_memories_favorite
  on public.love_space_memories(love_space_id, is_favorite, memory_date);
create index if not exists idx_love_space_bucket_due
  on public.love_space_bucket_items(love_space_id, is_done, target_date);
...
begin
  ...
  case p_record_type
    ...
    when 'memory' then
      -- A10: + emoji pilihan, tag, favorit.
      insert into public.love_space_memories as m(
        id,love_space_id,created_by,title,memory_date,notes,emoji,tags,is_favorite
      ) values(
        p_record_id,p_love_space_id,auth.uid(),left(trim(p_payload->>'title'),160),
        (p_payload->>'memory_date')::date,left(coalesce(p_payload->>'notes',''),4000),
        left(coalesce(p_payload->>'emoji',''),8),left(coalesce(p_payload->>'tags',''),400),
        coalesce((p_payload->>'is_favorite')::boolean,false)
      ) on conflict(id) do update set title=excluded.title,memory_date=excluded.memory_date,
        notes=excluded.notes,emoji=excluded.emoji,tags=excluded.tags,
        is_favorite=excluded.is_favorite,updated_at=now()
    ...
    when 'bucket_item' then
      -- A10: + catatan & prioritas (0–3).
      insert into public.love_space_bucket_items as b(
        id,love_space_id,created_by,title,category,target_date,notes,priority,is_done,completed_at
      ) values(
        ...
        greatest(0,least(3,coalesce((p_payload->>'priority')::integer,0))),
        ...
        target_date=excluded.target_date,notes=excluded.notes,priority=excluded.priority,
        is_done=excluded.is_done,updated_at=now(),
