...
export const GameProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const deleteHabit = useCallback((id: string) => {
    rpg.deleteHabit(id).then((res) => applyLive(res)).catch(() => setHabits((prev) => prev.filter((h) => h.id !== id)));
  }, [applyLive])
  ...
  const deleteDaily = useCallback((id: string) => {
    rpg.deleteDaily(id).then((res) => applyLive(res)).catch(() => setDailies((prev) => prev.filter((d) => d.id !== id)));
  }, [applyLive])
  ...
  const deleteQuest = useCallback((id: string) => {
    rpg.deleteQuest(id).then((res) => applyLive(res)).catch(() => setQuests((prev) => prev.filter((q) => q.id !== id)));
  }, [applyLive])
