...
export const SuppliesView: React.FC<{ onNavigate?: (tab: any) => void }> = ({ onNavigate }) => {
  const { lang, showToast, user } = useGame();
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-3 bg-slate-900/80 border border-slate-800 p-5 rounded-2xl">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button onClick={() => onNavigate?.('economy')} className="px-4 py-2 rounded-xl bg-teal-600 text-white text-sm font-bold">
            💰 {t('supplies_open_economy', lang === 'id' ? 'Buka Ekonomi' : 'Open Economy')}
          </button>
          <button onClick={openAdd} className="px-4 py-2 rounded-xl bg-emerald-600 text-white text-sm font-bold flex items-center gap-1">
            <Plus className="w-4 h-4" /> {t('supplies_add', lang === 'id' ? 'Tambah' : 'Add')}
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          <input
            ...
            placeholder={t('supplies_search_ph', lang === 'id' ? 'Cari barang…' : 'Search items…')}
        ...
        <select value={catFilter} onChange={(e) => setCatFilter(e.target.value)} className="px-3 py-2.5 rounded-xl bg-slate-900/80 border border-slate-800 text-sm text-slate-100">
          <option value="">{t('supplies_all_categories', lang === 'id' ? 'Semua Kategori' : 'All Categories')}</option>
        ...
        <div className="rounded-2xl bg-amber-500/10 border border-amber-500/40 p-4 flex items-start gap-3">
          ...
          <div>
            <div className="text-sm font-bold text-amber-300">
              {t('supplies_low_banner', lang === 'id' ? 'Beberapa barang menipis:' : 'Some items are low:').replace('{n}', String(low))}
            </div>
            <div className="text-xs text-amber-200/80 mt-0.5">
              {items.filter((it) => it.minStock > 0 && it.stock <= it.minStock).slice(0, 5).map((it) => it.name).join(', ')}
              {low > 5 && (lang === 'id' ? ` dan ${low - 5} lagi…` : ` and ${low - 5} more…`)}
            </div>
          </div>
        </div>
      )}
      ...
      {cats.length > 0 && (
        <div className="rounded-2xl bg-slate-900/70 border border-slate-800 p-4">
          <h3 className="text-xs font-bold text-slate-300 mb-2">{t('supplies_per_category', lang === 'id' ? '📂 Per Kategori' : '📂 Per Category')}</h3>
      ...
      <div className="space-y-2">
        {items.length === 0 && <p className="text-sm text-slate-500">{t('supplies_empty', lang === 'id' ? 'Belum ada stok.' : 'No supplies yet.')}</p>}
        {items.map((it) => (
          <div
            key={it.id}
      ...
      <div className="rounded-2xl bg-slate-900/70 border border-slate-800 p-4">
        <h3 className="text-xs font-bold text-slate-300 mb-2">
          {t('supplies_history_title', lang === 'id' ? '🧾 Riwayat Transaksi' : '🧾 Transaction History')}
          {selected ? ` — ${selected.name}` : ''}
        </h3>
        {history.length === 0 ? (
          <p className="text-xs text-slate-500">{t('supplies_history_empty', lang === 'id' ? 'Belum ada riwayat.' : 'No history yet.')}</p>
        ) : (
