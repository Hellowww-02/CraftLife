  ...
  return (
    <div className="ct-backdrop fixed inset-0 z-[80] flex items-start justify-center pt-24" onClick={onClose}>
      ...
      >
        ...
        />
        <div className="ct-stagger max-h-72 overflow-y-auto p-2 space-y-1">
          {items.map((it) => (
            <button
              key={it.kind + it.id}
              type="button"
              className="ct-palette-item w-full text-left px-3 py-2 rounded-xl text-sm hover:bg-slate-800 text-slate-200"
              onClick={() => {
