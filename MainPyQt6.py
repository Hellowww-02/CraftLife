...
from PyQt6.QtGui import QColor, QDrag, QFont, QFontMetrics, QIcon, QPen, QTextCharFormat, QTextCursor, QPainter, QPixmap
...
def _emoji_label(text: str, px: int = 48) -> QLabel:
...
class TopBar(QWidget):
    def __init__(self, on_notif, on_profile, on_toggle_theme=None):
        ...
        self._logo.setFont(QFont("Segoe UI", 25, QFont.Weight.Bold))
...
class NavBar(QWidget):
    ...
    def _build(self):
        ...
        for key_label, key in self._TABS:
            ...
            b.setStyleSheet("text-align: center; padding: 4px 2px; font-size: 10px;")
...
class FolderWidget(QWidget):
    ...
    def _build(self):
        ...
        ico_lbl.setFont(QFont("Segoe UI Emoji", 40))
...
class SportTrackPage(QWidget):
    ...
    def _build_card_content(self, item: dict) -> QWidget:
        ...
        ico = _emoji_label(item.get("icon") or sport_data["icon"], 54)
        row.addWidget(ico)
...
class PetsPage(QWidget):
    ...
    def load(self):
        ...
        for i, p in enumerate(pets):
            ...
            # Header baris
            top = QHBoxLayout()
            ico = _emoji_label(pet_data.get("icon", "🐾"), 54)
            top.addWidget(ico)
...
class GuildPage(QWidget):
    ...
    def _make_member_card(self, member: dict, current_user: dict, guild: dict, is_leader: bool) -> QFrame:
        ...
        avatar.setFont(QFont("Segoe UI", 30))
...
class DashboardPage(QWidget):
    ...
    def _stat_card(self, icon: str, title: str, value: str, subtitle: str = "", color: str = None):
        ...
        icon_lbl.setFont(QFont("Segoe UI", 50))
    ...
    def _rank_card(self, rank_data: dict) -> QWidget:
        ...
        # Icon besar
        icon = QLabel(rank_data["rank_icon"])
...
class AchievementPage(QWidget):
    ...
    def _make_achievement_card(self, ach: dict) -> QFrame:
        ...
        icon.setFont(QFont("Segoe UI", 35))
...
class SettingsPage(QWidget):
    ...
    def _build(self):
        ...
        for key, td in db.THEMES.items():
            row = QHBoxLayout()
            preview = QLabel("●")
            preview.setStyleSheet(f"color:{td['glow']}; font-size:22px;")
...
class NotesPage(QWidget):
    ...
    def _load_folder_tree(self):
        ...
        self._select_tree_item_by_id(-1)
    ...
    def _clear_editor(self):
        ...
        if hasattr(self, 'zoom_value_label') and self.zoom_value_label is not None:
            try:
                ...
                pass
        self._apply_zoom_to_editor(100)

    def _apply_zoom_to_editor(self, zoom_percent):
        """Ubah ukuran font seluruh isi editor sesuai zoom_percent (tidak permanen)."""
        if not hasattr(self, 'content_edit') or self.content_edit is None:
            return
        try:
            # Base size 12px (sesuai default font)
            base_size = 12
            new_size = base_size * zoom_percent / 100
            font = self.content_edit.font()
            font.setPointSizeF(new_size)
            # Terapkan ke seluruh dokumen
            self.content_edit.selectAll()
            self.content_edit.setCurrentFont(font)
            # Hapus seleksi
    ...
    def _load_note(self, note_id):
        ...
        zoom = 100  # selalu reset ke 100% saat load
        if hasattr(self, 'zoom_slider') and self.zoom_slider is not None:
            try:
        ...
        if hasattr(self, 'zoom_value_label') and self.zoom_value_label is not None:
            try:
                self.zoom_value_label.setText(f"{zoom}%")
            except RuntimeError:
                pass
        # Terapkan zoom ke editor (reset ke 100%)
        self._apply_zoom_to_editor(zoom)
    ...
    # ========== ZOOM (tidak digunakan karena QTextEdit) ==========
    def _on_zoom_changed(self, value):
        if hasattr(self, 'zoom_value_label') and self.zoom_value_label is not None:
            try:
                ...
                pass
        self._apply_zoom_to_editor(value)
