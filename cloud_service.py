...
class CloudService:
    ...
    def save_love_space_shared_profile(self, love_space_id: str, values: dict):
        # FIX: server versi baru (migrasi phase4f) menerima data DUA sisi
        # (my_*, partner_birthdate). Server lama hanya 5 param. Kirim penuh dulu;
        # bila server belum di-migrasi, fallback ke signature lama agar profil
        # tetap tersimpan (data sisi-kamu disimpan lokal oleh caller).
        full = {
            ...
            "p_my_name":values.get("my_name", ""),
            "p_my_gender":values.get("my_gender", "male"),
            "p_my_age":int(values.get("my_age") or 25),
            "p_my_birthdate":values.get("my_birthdate") or None,
            "p_partner_birthdate":values.get("partner_birthdate") or None,
        }
        try:
            return self.rpc("save_love_space_shared_profile", full)
        except Exception as error:
            if "could not find the function" not in str(error).lower():
                raise
            legacy = {k: v for k, v in full.items()
                      if k in ("p_love_space_id","p_partner_name","p_partner_gender",
                               "p_partner_age","p_relationship_type","p_start_date")}
            return self.rpc("save_love_space_shared_profile", legacy)
    ...
    def record_productivity_event(self, payload: dict):
        # FIX: migrasi Phase 5A mengganti signature RPC ini menjadi TANPA prefix p_
        # (PostgREST mencocokkan NAMA parameter). Kirim tanpa prefix; bila server
        # masih versi lama (prefix p_), coba ulang dengan prefix agar tetap kompatibel.
        plain = {
            "event_type": payload["event_type"],
            "source_local_id": str(payload.get("source_local_id", "")),
            "idempotency_key": payload["idempotency_key"],
            "completed_at": payload["completed_at"],
            "device_id": payload.get("device_id", "CraftLife Desktop"),
            "payload": payload.get("payload", {}),
        }
        try:
            return self.rpc("record_productivity_event", plain)
        except Exception as error:
            if "could not find the function" not in str(error).lower():
                raise
            prefixed = {f"p_{k}": v for k, v in plain.items()}
            return self.rpc("record_productivity_event", prefixed)
